...
mod erspan {
    include!(concat!(env!("OUT_DIR"), "/erspan.skel.rs"));
}
use erspan::*;

#[derive(Debug, Parser)]
struct Command {
    /// Interface index to attach tc program
...
fn main() -> Result<()> {
    ...
    let skel_builder = ErspanSkelBuilder::default();
    let open_skel = skel_builder.open()?;
    let mut skel = open_skel.load()?;
    let progs = skel.progs();
    let ifidx = nix::net::if_::if_nametoindex(opts.iface.as_str())? as i32;

    let mut tc_builder = TcHookBuilder::new(progs.erspan_get_tunnel().as_fd());
    tc_builder
        .ifindex(ifidx)
        .replace(true)
        .handle(1)
    ...
    let mut destroy_all = libbpf_rs::TcHook::new(progs.erspan_get_tunnel().as_fd());
    destroy_all
        .ifindex(ifidx)
    ...
    if opts.query {
        ...
        match ingress.query() {
            Err(e) => println!("failed to find ingress hook: {e}"),
            Ok(prog_id) => println!("found ingress hook prog_id: {prog_id}"),
        }
    }
